...
window.onload = function() {
  ...
  askForCookiePermission(); // Chama a função para solicitar a autorização de cookies
};
...
function acceptCookies() {
  localStorage.setItem('cookieConsent', 'true');
  document.body.removeChild(cookieAlert);
  exibirPopup(basicPopUp, 'popupShown', () => document.cookie.indexOf("popupShown=true") === -1); // Mostra o popup original após aceitar os cookies
  // Defina o cookie de popupShown aqui, se necessário
  document.cookie = "popupShown=true; expires=Fri, 31 Dec 9999 23:59:59 GMT; path=/";
}
