...
const resultadoPopUp = {
  ...
  headingText: {
      ...
      class: "",
      id: "",
      textContent: "Veja aqui os resultados do questionário de programação!",
      onClick: "",
  },
  linkButton: {
      tag: "a",
...
function exibirResultadoPopUp() {
  const resultadoPopUpElement = createPopUp(resultadoPopUp);
  const mainElement = document.getElementById('main');
  mainElement.appendChild(resultadoPopUpElement);
}

window.onload = function() {
  if (document.cookie.indexOf("resultadoPopUpShown=true") === -1) {
...
function closePopUp(parentElement) {
  ...
  removeOverlay();
  if (localStorage.getItem('cookieConsent') === 'true') {
      document.cookie = "resultadoPopUpShown=true; expires=Fri, 31 Dec 9999 23:59:59 GMT; path=/";
  }
}
